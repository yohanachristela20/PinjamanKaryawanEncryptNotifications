...
function MasterUser() {
  ...
  const getUser = async () =>{
    try {
      const response = await axios.get("http://10.70.10.110:5000/user", {
        headers: {
  ...
  const deleteUser = async(id_user) =>{
    try {
      await axios.delete(`http://10.70.10.110:5000/user/${id_user}` , {
        headers: {
          Authorization: `Bearer ${token}`,
      },
      }); 
  ...
  const setPassword = async(id_user) => {
    try {
        ...
        await axios.put(`http://10.70.10.110:5000/user/${id_user}`, {
        }, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
