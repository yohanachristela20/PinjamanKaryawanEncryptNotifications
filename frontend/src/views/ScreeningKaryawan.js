...
function ScreeningKaryawan({ setHasilScreening }) {
    ...
    const fetchData = async () => {
      ...
      try {
        const [
          ...
          await axios.get(
          `http://10.70.10.119:5000/plafond-update-saat-ini/${selectedPinjaman.id_pinjaman}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const updatePinjamanStatus = (status) => {
    axios.put(`http://10.70.10.119:5000/pinjaman/cancel/${selectedPinjaman.id_pinjaman}`, {
